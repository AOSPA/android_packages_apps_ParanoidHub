...
public class HubUpdateManager implements ClientConnector.ConnectorListener {
    ...
    private void fetchOrUpdateDeviceChangelog() {
        if (mEnabled) {
            ...
            mConnector.insert(oldJson, newJson, url);
            mIsDeviceChangelogMatchMaking = true;
            beginMatchMaker();
        }
    }
    ...
    public void onClientStatusSuccess(File oldJson, File newJson) {
        if (mIsConfigMatchMaking) {
            ...
            fetchOrUpdateChangelog();
            fetchOrUpdateBetaChangelog();
        ...
        if (mIsBuildBetaChangelogMatchMaking) {
            try {
                mBetaChangelog = UpdatePresenter.matchMakeBuildBetaChangelog(oldJson, newJson);
            } catch (IOException | JSONException e) {}
